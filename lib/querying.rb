...
def select_series_title_with_most_human_characters
  "SELECT Series.title FROM Series INNER JOIN Authors ON Series.author_id = Authors.id INNER JOIN Characters ON Author.id = Characters.author_id GROUP BY Characters.species ORDER BY COUNT(*) DESC LIMIT 1"
end
