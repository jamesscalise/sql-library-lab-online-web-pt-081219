...
def select_character_names_and_number_of_books_they_are_in
  "SELECT Characters.name, COUNT(Books.title) FROM Characters INNER JOIN character_books ON Characters.id = character_books.character_id INNER JOIN Books ON Books.id = character_books.book_id GROUP BY Characters.name"
end
